...
CORNER_TOTAL_THR = 2750     # tune via UI (MODE_BETWEEN)
...
gyro_kd = 4
gyro_max_speed = 1500
LINE_BLACK_SENSOR_THR = 700   # tune 650–800 based on calibration
...
# NEW: timing + mode flag for "first 2 sensors" behaviour in MODE_LINE
LINE_EDGE_DELAY_MS = 3000   # 13 seconds before switching to edge-follow
line_mode_start_ms = 0       # when we entered MODE_LINE
use_edge_mode = False        # False = normal 5-sensor follow, True = first 2 sensors
EDGE_TARGET_POS = 500        # desired position (0..1000) for edge mode; tune if needed
...
def update_display(p, selected_param, mode):
    ...
    if mode == MODE_WAIT:
        ...
        # NEW: show when we're in edge mode
        if use_edge_mode:
            state = "MODE: LINEE"   # LINE Edge
        else:
            state = "MODE: LINE"
    else:
        state = "MODE: BETWEEN"
...
def main():
    ...
    global CORNER_WHITE_THR, CORNER_TOTAL_THR
    global line_mode_start_ms, use_edge_mode   # NEW
    ...
    while True:
        ...
        if any_pressed and not bump_was_pressed:
            if mode == MODE_WAIT:
                ...
                # NEW: start 13s timer and reset edge mode
                line_mode_start_ms = now_ms
                use_edge_mode = False
                print("ENTER MODE_LINE, timer started")
                ...
                use_edge_mode = False  # reset, next LINE run starts from centre
        ...
        if mode == MODE_LINE:
            # NEW: after 13s in MODE_LINE, switch to first-2-sensor following
            if not use_edge_mode:
                if time.ticks_diff(now_ms, line_mode_start_ms) >= LINE_EDGE_DELAY_MS:
                    use_edge_mode = True
                    print("EDGE MODE ENABLED: using sensors 0 & 1")

            # MODE_LINE: robot chases black line
            if use_edge_mode:
                # Use only sensors 0 & 1 (leftmost two)
                s0 = line[0]
                s1 = line[1]
                total_01 = s0 + s1

                if total_01 > LINE_TOTAL_MIN:
                    # Position in 0..1000 between sensor 0 and 1
                    pos01 = (0 * s0 + 1000 * s1) // total_01
                    p_raw = pos01 - EDGE_TARGET_POS
                else:
                    # If left sensors see nothing, fall back to centre error
                    p_raw = l - 2000
            else:
                # Original behaviour: 5-sensor centre following
                p_raw = l - 2000
            ...
            if (not has_black and
                ...
                p = 0
                last_l = 2000
                # NOTE: we do NOT reset line_mode_start_ms or use_edge_mode here.
                # Edge mode will stay active once enabled for this MODE_LINE run.
